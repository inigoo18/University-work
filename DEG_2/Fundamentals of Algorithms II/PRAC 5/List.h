...
public:
	...
	*/
	...
	void sortOut(int fingers) {
		int pos = 0;
		while (_numElems > 1) {
			pos = takePersonOut(pos, fingers);
		}
		print();
	}

	// Implemented method:
	int takePersonOut(int pos, int fingers) {
		int times = fingers;
		int i = 0;
		List <int>::MutableIterator it = begin();
		for (int j = 0; j < pos; j++) {
			it.next();
			i++;
		}
